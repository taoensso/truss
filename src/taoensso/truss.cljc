  ...
  ([arg opts]
   (throw
     (ex-info (or (get opts :msg) (str "Unexpected argument: " (if (nil? arg) "<nil>" arg)))
       (conj {:arg (impl/typed-val arg)} (dissoc opts :msg)))))
    ...
    ([error-fn rf]
     (let [error-fn
       ...
       (fn catching-rf
         ...
         ([acc    ] (try* (rf acc)    (catch :all t (error-fn {:rf rf :call '(rf acc)    :args {:acc (impl/typed-val acc)}} t))))
         ([acc in ] (try* (rf acc in) (catch :all t (error-fn {:rf rf :call '(rf acc in) :args {:acc (impl/typed-val acc)
                                                                                                :in  (impl/typed-val in)}} t))))
         ([acc k v]
          (try* (rf acc k v)
            (catch :all t
              (error-fn
  ...
  (defn failed-assertion-ex-info
    ...
    ([legacy-ex-data? failed-assertion-info]
     (let [{:keys [inst ns coords, pred arg-form arg-val, data error]} failed-assertion-info
           ...
           msg (str "Truss assertion failed at " ns coords ": " (list pred arg-form))
           msg
           (if error
...
(defn ^:no-doc failed-assertion!
  "Private, don't use."
  [ns-id coords, pred arg-form arg-val, data-fn error]
  (if-let [;; Not accessible from impl ns in Cljs
    ...
    (handler
      (let [undefined-arg? (instance? ArgEvalError arg-val)
            ns
            #?(:clj  (str *ns*) ; Runtime ns okay
               :cljs (get @impl/ns-registry_ ns-id "unknown"))]
        (FailedAssertionInfo. ns coords pred arg-form
...
#?(:clj
   (defmacro have
     ...
     [& args] `(impl/assert-args true false ~(impl/callsite-coords &form) ~args)))
...
#?(:clj
   (defmacro have?
     ...
     {:arglists '([x] [pred (:in) x] [pred (:in) x & more-xs])}
     [& args] `(impl/assert-args true true ~(impl/callsite-coords &form) ~args)))
...
#?(:clj
   (defmacro have!
     ...
     {:arglists '([x] [pred (:in) x] [pred (:in) x & more-xs])}
     [& args] `(impl/assert-args false false ~(impl/callsite-coords &form) ~args)))
...
#?(:clj
   (defmacro have!?
     ...
     **WARNING**: do NOT use in `:pre`/`:post` conditions since those ALWAYS
     ...
     {:arglists '([x] [pred (:in) x] [pred (:in) x & more-xs])}
     [& args] `(impl/assert-args false true ~(impl/callsite-coords &form) ~args)))
