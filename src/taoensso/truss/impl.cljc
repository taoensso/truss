...
#?(:clj (defmacro typed-val [x] `{:value ~x, :type (type ~x)}))
...
(defn revery? #?(:cljs {:tag 'boolean}) [pred coll] (reduce (fn [acc in] (if (pred in) true (reduced false))) true coll))
...
(defn ks=      #?(:cljs {:tag 'boolean}) [ks-set m] (=             (set (keys m))   ks-set))
(defn ks<=     #?(:cljs {:tag 'boolean}) [ks-set m] (set/subset?   (set (keys m))   ks-set))
(defn ks>=     #?(:cljs {:tag 'boolean}) [ks-set m] (set/superset? (set (keys m))   ks-set))
...
#?(:clj
   (defn callsite-coords                   [macro-form]
     (when-let [{:keys [line column]} (meta macro-form)]
       (when line      [line column]))))

(defonce ^{:doc "{id ns-string, ns-string id}"} ns-registry_ (volatile! {}))
(defonce ^{:doc "Returns registry id for current/given namespace."}
  ns-registry-id!
  (let [id_ (volatile! 0)]
    (fn      ns-registry-id!
      ([  ] (ns-registry-id! *ns*))
      ([ns]
       (let [ns-str (str ns)]
         (or
           (get @ns-registry_ ns-str)
           (let [id (vswap! id_ inc)]
             (vswap! ns-registry_ (fn [m] (assoc m id ns-str, ns-str id)))
             id)))))))
...
#?(:clj
   (defmacro assert1
     [bool? coords-form [psafe? pform pshow] arg-form data-fn-form]
     (let [cljs?       (:ns &env)
           coords-form (or coords-form (callsite-coords &form))
           eval-arg?   (list-form? arg-form)
           ns-id       (when cljs? (ns-registry-id! (str *ns*)))
       ...
       (case [(if eval-arg? :eval-arg  :local-arg)
         ...
         `(if (~pform ~arg-form)
            ...
            (taoensso.truss/failed-assertion! ~ns-id ~coords-form ~pshow '~arg-form ~arg-form ~data-fn-form nil))

         [:local-arg :unsafe-pred]
         `(let [~gs-error (catching (if (~pform ~arg-form) nil FalsePredError) ~'e ~'e)]
            (if ~gs-error
              (taoensso.truss/failed-assertion! ~ns-id ~coords-form ~pshow '~arg-form ~arg-form ~data-fn-form ~gs-error)
              ~(if bool? true arg-form)))
         ...
         `(let [~gs-arg-val (catching ~arg-form ~'e (ArgEvalError. ~'e))
            ...
            (if ~gs-error
              (taoensso.truss/failed-assertion! ~ns-id ~coords-form ~pshow '~arg-form ~gs-arg-val ~data-fn-form ~gs-error)
...
#?(:clj
   (defmacro assert-args
     ...
     [elidable? bool? coords-form arg-forms]
     (let [bang?      (= (first arg-forms) :!) ; For back compatibility, undocumented
       ...
       (if elide?
         ...
         (case [(if in?       :in       :not-in)
           ...
           `(assert1 ~bool? ~coords-form [~psafe? ~pform '~pshow] ~?x1 ~data-fn-form)

           [:not-in :multi-x] ; (have* pred x1 x2 ...) -> [x1 x2 ...] or bool
           (let [body (mapv (fn [x] `(assert1 ~bool? ~gs-cf [~psafe? ~gs-pf ~gs-ps] ~x ~gs-df)) ?xs)
                 body (if bool? `(do ~@body true) body)]
             `(let [~gs-cf ~coords-form
                    ~gs-ps '~pshow
                    ~gs-pf ~pform
                    ~gs-df ~data-fn-form]
                ~body))
           ...
           (let [rfn (if bool? `revery? `revery)]
             `(let [~gs-cf ~coords-form
                    ...
                    ~gs-pf ~pform
                    ~gs-df ~data-fn-form]
                (~rfn (fn [~gs-in] (assert1 ~bool? ~gs-cf [~psafe? ~gs-pf ~gs-ps] ~gs-in ~gs-df)) ~?x1)))

           [:in :multi-x] ; (have* pred :in xs1 xs2 ...) -> [xs1   ...] or bool
           (let [rfn  (if bool? `revery? `revery)
                 body (mapv (fn [xs] `(~rfn (fn [~gs-in] (assert1 ~bool? ~gs-cf [~psafe? ~gs-pf ~gs-ps] ~gs-in ~gs-df)) ~xs)) ?xs)
                 body (if bool? `(do ~@body true) body)]
             `(let [~gs-cf ~coords-form
                    ~gs-ps '~pshow
                    ~gs-pf ~pform
